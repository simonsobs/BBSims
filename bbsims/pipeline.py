...
def check_and_fix_config_file(params):
    ...
    try: params.band_int
        ...
        params.band_int = False
        print_rnk0('WARNING: setting band_int=False', rank)
    ...
    try: params.make_fg
        ...
        params.make_fg = False
        print_rnk0('WARNING: setting make_fg=False', rank)
    try: params.gaussian_fg
    except:
        params.gaussian_fg = False
    if params.gaussian_fg:
        try: params.nmc_fg
        except: params.nmc_fg = None
        if params.nmc_fg == 1:
            params.nmc_fg = None
        params.fg_models = {
...
def write_inst_file(params):
    ...
    if params.gaussian_smooth==False:
        for k in instr:
            del instr[k]['beam']
    if params.band_int==False:
        for k in instr:
            del instr[k]['freq_band']
